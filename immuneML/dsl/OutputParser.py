...
class OutputParser:
    ...
    def parse(specs: dict, symbol_table: SymbolTable) -> dict:
        if "output" in specs:
            ParameterValidator.assert_keys(specs["output"], ["format", "destination"], "OutputParser", "output")
            ParameterValidator.assert_in_valid_list(specs["output"]["format"], ["HTML"], "OutputParser", "format")
            ParameterValidator.assert_in_valid_list(specs["output"]["destination"], ["MLflow"], "OutputParser", "destination")
        else:
